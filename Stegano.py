...
class Stegno:
    ...
    def pwd(self,f):
        ...
        a=tkinter.simpledialog.askstring("Password", "Enter password:", show='*')
        #f = Frame(root, bg="#DC143C")
        f.destroy()
        if a == password:
            self.frame1_encode(f)
        else:
    ...
    def pwd1(self,f):
        ...
        a1 = tkinter.simpledialog.askstring("Password", "Enter password:", show='*')
        f.destroy()
        #f = Frame(root, bg="#DC143C")
        if a1==password:
            self.frame1_decode(f)
        else:
            messagebox.showerror("Error", "Enter the correct password !")
    ...
    def main(self, root):
        ...
        title.config(font=('courier', 33), bg='#88cffa')
        title.grid(pady=10)

        b_encode = Button(f, text="Encode", command=lambda: self.pwd(f), padx=14)
        b_encode.config(font=('courier', 14))
        b_decode = Button(f, text="Decode", command=lambda: self.pwd1(f), padx=14)
        b_decode.config(font=('courier', 14))
        b_decode.grid(pady=12)

        c = Label(f, padx=50, pady=100, text="\'\(°Ω°)/\'")
        c.config(font=('courier', 50), bg='#88cffa')
        c.grid(padx=16)
    ...
    def frame1_decode(self, f):
        ...
        label_art.config(font=('courier', 90), bg='#FF00FF')
    ...
    def frame1_encode(self, f):
        ...
        #f.destroy()
        f2 = Frame(root)
        label_art = Label(f2, text='٩(^‿^)۶')
        label_art.config(font=('courier', 70), bg='#9932CC')
        label_art.grid(row=1, pady=60, padx=70)
